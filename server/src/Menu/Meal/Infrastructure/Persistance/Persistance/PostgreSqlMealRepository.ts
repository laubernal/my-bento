...
export class PostgreSqlMealRepository implements IMealRepository {
    ...
    public async find(filter: MealFilter): Promise<Meal[]> {
        try {
            ...
            const query = `
                ...
                SELECT paginated_meals.id,
                       paginated_meals.name,
                       paginated_meals.type,
                       paginated_meals.created_at,
                       paginated_meals.updated_at,
                       meal_foods.id         AS mealFood_id,
                       meal_foods.meal_id    AS mealFood_meal,
                       meal_foods.food_id    AS mealFood_food,
                       meal_foods.amount     AS mealFood_amount,
    ...
    public async save(entity: Meal): Promise<void> {
        try {
            ...
            const insertMealQuery = `INSERT INTO meals(${columns})
                                     VALUES (${values});`;
            ...
            for (const food of foods) {
                ...
                const insertMealFoodQuery = `INSERT INTO meal_foods(${columns})
                                             VALUES (${values})`;
    ...
    public async update(entity: Meal): Promise<void> {
        try {
            ...
            const updateMealQuery = `UPDATE meals
                                     SET ${setClause}
                                     WHERE id = '${mealModel.id}';`;
            ...
            for (const food of foods) {
                ...
                const updateMealFoodQuery = `UPDATE meal_foods
                                             SET ${setClause}
                                             WHERE id = '${food.id}';`;
    ...
    public async delete(entity: Meal): Promise<void> {
        try {
            ...
            for (const food of foods) {
                const deleteMealFoodQuery = `DELETE
                                             FROM meal_foods
                                             WHERE id = '${food.id}';`;
